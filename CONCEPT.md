...
- Each cycle, natural disasters have a chance to occur, **weighted by the
  ...
  Baseline weights per terrain/disaster pairing should be derived from
  real-world historical records for that terrain type once the set of
  base terrain types is finalized, rather than invented arbitrarily.
...
- Disasters affect each other's weights within the same cycle — e.g. a
  ...
  defined once disaster types are finalized.
...
- Source/method for deriving historical terrain-disaster weight data.
- Exact decay curve shape for post-disaster repeat-suppression.
- Full cross-disaster weight-interaction mapping (which disaster types
